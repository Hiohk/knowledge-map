...
* JVM进程会首先启动一个主线程（main-thread），主线程负责调用 main 方法。因此 main 方法是在主线程中运行的。
...
2. 如上图所示，假设只有一个 CPU 资源，线程之间要竞争得到执行机会。图中的第一个阶段，在 A 执行的过程中，B、C 不会执行，因为这段时间内这个 CPU 资源被 A 竞争到了，同理，第二阶段只有B在执行，第三阶段只有 C 在执行。其实，并发过程中，A、B、C 并不是同时进行的（微观角度），但又是同时进行的（宏观角度）。
...
* 资源分配不当：例如，线程1持有资源2，线程2持有资源1，但他们同时都想申请对方的资源，因此这两个线程会相互等待而进入死锁状态。
...
也可以使用 Lock 来实现线程安全，Lock是接口，从 JDK5 开始引入。Lock 接口下有一个实现类，可重入锁 `ReentrantLock` 。
