...
- **问题：能不能使用 count(列名)替换 count(\*)?**
  ...
  不要使用 count(列名)来替代 count(_) ， count(_) 是 SQL92 定义的标准统计行数的语法，跟数
