...
async function startChat(question: string) {
  ...
  try {
    ...
    await streamToString(response.body, assistantId);
  } catch (err) {
    console.log(err);
  }
}
...
async function streamToString(body: any, assistantId: number) {
  ...
  const decoder = new TextDecoder(); // 使用 TextDecoder 解码

  let currentAssistantContent = ""; // 当前正在构建的助手内容
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        try {
          ...
          const data = JSON.parse(line); // 解析 JSON 数据
          const content = data.result; // 提取 result 字段
          const assistantIndex = questions.value.findIndex(
            (q) => q.role === "assistant" && q.id === assistantId
          );

          // 如果尚未创建助手的回复条目，先创建一个条目
          if (assistantIndex === -1) {
            currentAssistantContent += content; // 逐步构建助手内容
            ...
            // 如果已存在助手条目，则逐步追加到当前助手内容
            currentAssistantContent += content; // 累加当前数据块内容
            questions.value[assistantIndex].content = currentAssistantContent; // 实时更新内容
          }
            ...
            <Combobox @update:modelValue="onSelect">
              <div class="chat-container">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                  stroke="currentColor" class="magnifying-glass-icon" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                </svg>
                <ComboboxInput class="chat-input" :placeholder="placeholder" aria-autocomplete="false"
                  @change="query = $event.target.value" @keyup.enter="keyEnter" :value="query" autocomplete="off" />
              </div>
              <ComboboxOptions v-if="props.predefinedQuestions.length > 0 && !questions.length" static
                class="combobox-options">
                <li v-if="props.predefinedQuestions.length > 0">
                  <ul class="combobox-options-container">
                    <ComboboxOption v-for="action in filteredPredefinedQuestions" :key="action" :value="action"
              ...
              <div v-if="
                props.predefinedQuestions.length === 0 &&
                questions.length === 0
              " class="result-not-found">
                <p class="result-not-found-text">How can I help you today?</p>
              </div>
                ...
                <ul class="question-anwser-section">
                  <li class="question-anwser-item" v-for="(q, index) in questions" v-bind:key="index">
                    <div v-if="q.role === 'user'" class="question-role-user">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="question-role-icon">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="anwser-content" v-else-if="q.role === 'assistant' && q.content">
                      <svg class="question-role-icon documate-logo" viewBox="0 0 24 24" fill="none"
                      ...
                      <div class="markdown-body" v-html="markdownToHtml(q.content)"></div>
                    </div>
                  </li>
                  <li>
                    <div v-show="loading" class="loading">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="loading-spin">
                        <path stroke-linecap="round" stroke-linejoin="round"
              ...
              <div class="footer">
                ...
                <a href="https://github.com/Hiohk" class="powered-by" target="_blank">
